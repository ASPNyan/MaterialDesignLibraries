namespace MaterialDesign.Temperature;
...
public struct TemperatureCache(HCTA input)
{
    ...
    public static double RawTemperature(HCTA color)
    {
        ...
        double hue = Color.SanitizeDegrees(Math.Atan2(lab.B, lab.A) * 180 / double.Pi);
        
        double chroma = Math.Sqrt(Math.Pow(lab.A, 2) + Math.Pow(lab.B, 2));

        return -0.5 * 0.02 * Math.Pow(chroma, 1.07) * Math.Cos(Color.SanitizeDegrees(hue - 50) * double.Pi / 180);
    }
    ...
    public HCTA GetComplement()
    {
        ...
        HCTA answer = GetHCTAsByHue()[Color.Round0(Input.H)];

        double complementRelativeTemp = 1 - GetRelativeTemperature(Input);
        for (double hueAddend = 0; hueAddend <= 360; hueAddend++)
        {
            double hue = Color.SanitizeDegrees(startHue + directionOfRotation * hueAddend);
            if (!IsBetween(hue, startHue, endHue)) continue;

            HCTA possibleAnswer = GetHCTAsByHue()[Color.Round0(hue)];
    ...
    {
        int startHue = Color.Round0(Input.H);
        ...
        for (int i = 0; i < 360; i++)
        {
            int hue = Color.SanitizeDegrees(startHue + i);
        ...
        {
            int hue = Color.SanitizeDegrees(startHue + hueAddend);
            HCTA hcta = GetHCTAsByHue()[hue];
