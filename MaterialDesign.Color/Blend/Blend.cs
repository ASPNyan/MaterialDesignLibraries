...
public static class Blend
{
    ...
    {
        double differenceDegrees = Colorspaces.Color.DifferenceDegrees(designColor.H, sourceColor.H);
        double rotationDegrees = Math.Min(differenceDegrees * 0.5, 15);

        double outputHue =
            Colorspaces.Color.SanitizeDegrees(designColor.H + rotationDegrees * RotationDirection(designColor.H, sourceColor.H));
    ...
    private static int RotationDirection(double from, double to) => Colorspaces.Color.SanitizeDegrees(to - from) <= 180 ? 1 : -1;
}
